...
const EditPaket = () => {
  ...
  useEffect(() => {
    const getPaket = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/paket/${id}`);
  ...
  const editPaket = async (e) => {
    ...
    try {
      const response = await axios.patch(`${process.env.REACT_APP_BASE_URL}/paket/${id}`, postPaket, { withCredentials: true, headers: { Authorization: `Bearer ${token}` } });
